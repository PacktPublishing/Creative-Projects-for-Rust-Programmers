...
use actix_web::{HttpMessage, Error, AsyncResponder};
...
fn upload_specified_file(req: &HttpRequest)
-> Box<Future<Item = HttpResponse, Error = Error>>
{
    ...
    req.body().from_err().and_then(move |contents| {
        // Create the file.
...
fn upload_new_file(req: &HttpRequest)
-> Box<Future<Item = HttpResponse, Error = Error>>
{
    let info = req.match_info();
    ...
    req.body().from_err().and_then(move |contents| {
        ...
        loop {
            ...
            filename = format!("{}{:03}.txt",
                filename_prefix, rng.gen_range(0, 1000));
