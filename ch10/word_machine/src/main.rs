...
fn execute(program: &[u16]) -> u16 {
    ...
    loop {
        ...
        17 => // jump_if_negative
            if (acc as i16) < 0 { ip = operand as usize },
        18 => // jump_if_nonpositive
            if (acc as i16) <= 0 { ip = operand as usize },
        19 => // jump_if_nonnegative
            if (acc as i16) >= 0 { ip = operand as usize },
        _ => {}
        }
    }
...
fn main() {
    ...
    // Ask the user to type a line, and convert it to a number, until you can.
    /*
    let mut buf = vec![0u16; 5];
    input_line(&mut buf);
    let mut sum = 0u16;
    for i in 0..buf.len() {
        let digit = buf[i].wrapping_sub(48);
        if digit > 9 { break; }
        sum = sum.wrapping_mul(10).wrapping_add(digit);
    }
    println!("{}", sum);
*/
/*
    61, // 0:
    // Let the user input the digits.
    1, 51, // 1: set digits
    6, 5, // 3: input 5
...
// 9: before_parsing_number
    // If the digit is less than 0, parsing is ended.
    4, 56, // 9: indirect_load pos
    9, 58, // 11: subtract ascii_zero
    17, 49, // 13: jump_if_negative after_parsing_number

    // If the digit is greater than 9, parsing is ended.
    4, 56, // 15: indirect_load pos
    9, 58, // 17: subtract ascii_zero
    9, 60, // 19: subtract number_base
    19, 49, // 21: jump_if_nonnegative after_parsing_number

    // Multiply by 10 the current sum.
    2, 57, // 23: load sum
    10, 60, // 25: multiply number_base
    3, 57, // 27: store sum

    // Add next digit to current sum.
    4, 56, // 29: indirect_load pos
    9, 58, // 31: subtract ascii_zero
    8, 57, // 33: add sum
    3, 57, // 35: store sum
    ...
    // If pos points to itself, the digit buffer is ended.
    1, 56, // 43: set pos
    9, 56, // 45: subtract pos
    15, 9, // 47: jump_if_nonzero before_parsing_number

// 49: after_parsing_number
    0, 0, // 49: terminate 0

    0, 0, 0, 0, 0, // 51: digits: array 5
    0, // 56: pos: word 0
    0, // 57: sum: word 0
    48, // 58: ascii_zero: word 48
    1, // 59: one: word 1
    10, // 60: number_base: word 10
];
execute(&prog);
*/
