use actix_web::{http, server, App, Binary, FromRequest, HttpRequest, HttpResponse, Responder};
...
fn check_credentials(
    ...
    if let Ok(credentials) = BasicAuth::from_request(&req, &config) {
        ...
        if let Some(user) = db_conn.get_user_by_username(credentials.username()) {
        ...
        Err("Credentials missing.".to_string())
    }
}
...
fn get_page_login_with_message(_req: &HttpRequest<AppState>, error_message: &str) -> HttpResponse {
    let mut context = tera::Context::new();
