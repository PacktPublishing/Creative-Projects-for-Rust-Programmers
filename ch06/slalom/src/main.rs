use quicksilver::{
    Result,
    geom::{Rectangle, Circle, Triangle, Vector, Transform},
...
impl Screen {
    fn get_random_door(door_at_right: bool) -> (f32, f32) {
        ...
        let pole_pos = rng.gen_range(
            -DOOR_WIDTH / 2., SCREEN_WIDTH / 2. - DOOR_WIDTH * 1.5);
        if door_at_right {
    ...
    fn steer(&mut self, direction: i8) -> Result<()> {
...
impl State for Screen {
    ...
    fn update(&mut self, window: &mut Window) -> Result<()> {
        match self.mode {
            ...
            Mode::Running => {
                ...
                if self.elapsed_sec - self.elapsed_shown_sec
                    >= MIN_TIME_DURATION {
                ...
                let ski_tip_along = SCREEN_HEIGHT * 15. / 16.
                    - SKI_LENGTH / 2. - SKI_TIP_LEN;
                
                let ski_tip_across = SCREEN_WIDTH / 2.
    ...
    fn draw(&mut self, window: &mut Window) -> Result<()> {
        ...
        for i_door in self.disappeared_doors
            ..self.disappeared_doors + N_DOORS_IN_SCREEN {
            if i_door >= TOTAL_N_DOORS { break; }
            let door = self.doors[i_door];
            let pole_color = Background::Col(
                if i_door == TOTAL_N_DOORS - 1
                { Color::GREEN } else { Color::BLUE }
            );
            let doors_along_pos =
                self.doors_along_offset + SCREEN_HEIGHT /
                N_DOORS_IN_SCREEN as f32 *
                (self.disappeared_doors + N_DOORS_IN_SCREEN
                - 1 - i_door) as f32;
            window.draw(&Circle::new((
        ...
        let elapsed_shown_text = format!(
            ...
            TOTAL_N_DOORS - self.disappeared_doors
                - if self.entered_door { 1 } else { 0 },
            match self.mode {
